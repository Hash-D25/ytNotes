...
export default function ScreenshotsList({
  ...
  const fetchScreenshots = async () => {
    try {
      setLoading(true);
      console.log(
        "🔍 Frontend: Fetching screenshots for video:",
        video?.videoId
      );

      const response = await authAxios.get(
        `/bookmark/${video.videoId}/screenshots`
      );
  ...
  const handleDeleteScreenshot = async (screenshot) => {
    try {
      const response = await authAxios.delete(
        `/screenshots/${video.videoId}/${screenshot.timestamp}`
      );
      ...
      {screenshots.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sortedScreenshots.map((screenshot, idx) => (
            <div
              key={idx}
              className="youtube-card rounded-xl shadow-md border border-gray-700 overflow-hidden"
            >
              <div className="relative">
                <img
                  ...
                  onLoad={() =>
                    console.log(
                      "🔍 Frontend: Image loaded successfully:",
                      screenshot.path
                    )
                  }
                  onError={(e) => {
                    console.log(
                      "🔍 Frontend: Image failed to load:",
                      screenshot.path
                    );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-90 z-[9999999] flex items-center justify-center p-4 modal-overlay">
          <div className="relative max-w-4xl max-h-full">
            ...
            <img
              ...
              alt={`Screenshot at ${formatTimestamp(
        ...
        <div className="fixed inset-0 bg-black bg-opacity-90 z-[9999999] flex items-center justify-center p-4 modal-overlay">
          <div className="relative max-w-4xl max-h-full">
            ...
            <img
              ...
              alt={`Screenshot ${slideshowIndex + 1} of ${
            ...
            <div className="absolute bottom-0 left-0 right-0 h-1 bg-black bg-opacity-30">
              <div
                ...
                style={{
                  width: `${
                    ((slideshowIndex + 1) / sortedScreenshots.length) * 100
                  }%`,
                }}
              />
            </div>
